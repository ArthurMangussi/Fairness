...
class ModelsImputation:
    ...
    def model_knn_custom(dataset_train:pd.DataFrame, listTypes):
        imputer = CustomKNNImputer(n_neighbors=3, original=dataset_train, featuresType=listTypes)
        knn = imputer.fit(dataset_train.iloc[:, :].values)
                ...
                for nro_layers in [1, 2]:
                    if nro_layers == 1:
                        for nro_neurons in param_grid["neurons"]:
                            neurons = [nro_neurons]
                            vae_config = ModelsImputation.set_vae_config(X_train, neurons, n_latent_dimension, n_epochs)
    ...
    def model_missForest(dataset_train:pd.DataFrame):
        ...
        imputer = IterativeImputer(estimator=rf, max_iter=100)
        ...
        match model:
            ...
            case "pmivae":
                ...
                params = {
                        "epochs": [200],
                        "latent_dimension": [5],
                        "neurons": list(np.arange(60, 70, 10)),
                    }
