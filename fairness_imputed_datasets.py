...
def processa_imputados_fairness(path:str, model_impt:str, nome_datasets:list, mecanismo:str):
    ...
    for nome in nome_datasets:
        ...
        for mr in [10,20,40,60]:
            _logger.info("="*40)
            _logger.info(f"{model_impt} = {nome} com md = {mr}")
            ...
            os.makedirs(f"./Fairness/ClassificaçãoImputed/{mecanismo}/{nome}_{model_impt}_md{mr}", exist_ok=True)
            automl = AutoML(results_path=f"./Fairness/ClassificaçãoImputed/{mecanismo}/{nome}_{model_impt}_md{mr}",
                            algorithms=["Xgboost"],
                            mode="Optuna",
            ...
            _logger.info("Começando treinamento do modelo")
            ...
            _logger.info("Calcular métricas:")
            f1 = f1_score(y_true=y_imputed,
                  y_pred=y_pred_baseline)
    
            f1_metrics[f"{nome}_{model_impt}_md{mr}"] = f1
            results_metrics[f"{nome}_{model_impt}_md{mr}"] = metricas_imputed
